...
function visitBlock(fileBlock, block, opts, next){
    BLOCK_VISITORS_BY_TYPE[fileBlock.blobHeader.type](block, opts, next);
}

function visitOSMHeaderBlock(block, opts, next){
    next();
    // TODO
}

function visitOSMDataBlock(block, opts, next){
    var i, j;
    var items = [];
    var item, pg;
    for(j = 0; j < block.primitivegroup.length; ++j){
        pg = block.primitivegroup[j];
        for(i = 0; i < pg.nodesView.length; ++i){
            item = pg.nodesView.get(i);
            item.type = 'node';
            items.push(item);
        }
        for(i = 0; i < pg.waysView.length; ++i){
            item = pg.waysView.get(i);
            item.type = 'way';
            items.push(item);
        }
        for(i = 0; i < pg.relationsView.length; ++i){
            item = pg.relationsView.get(i);
            item.type = 'relation';
            items.push(item);
        }
        // visitPrimitiveGroup(block.primitivegroup[i], opts, next);
    }
    if (opts.data){
        opts.data(items, next);
    }else{
        next();
    }
}

function visitPrimitiveGroup(pg, opts, next){
    var i;
    var items = [];
    var item;
    for(i = 0; i < pg.nodesView.length; ++i){
