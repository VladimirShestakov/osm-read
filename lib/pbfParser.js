...
function visitPrimitiveGroup(pg, opts){
    ...
    if(opts.node){
        for(i = 0; i < pg.nodesView.length; ++i){
            opts.node(pg.nodesView.get(i), pg.nodesView.length);
        }
    }
    ...
    if(opts.way){
        for(i = 0; i < pg.waysView.length; ++i){
            opts.way(pg.waysView.get(i), pg.waysView.length);
        }
    }
    ...
    if(opts.relation){
        for(i = 0; i < pg.relationsView.length; ++i){
            opts.relation(pg.relationsView.get(i), pg.relationsView.length);
        }
    }
}
